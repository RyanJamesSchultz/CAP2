function [S]=EnsembleW(M,m1,S_m2,b,kd,Nr,reshuffle_flag,smooth_flag, GRtype_flag,NLEtype_flag,Sa,dMa,ba,m2a)
  ...
  for l=1:Nr+1
      ...
      Wl=getEW(Ml,m1,b,kd,S, GRtype_flag,NLEtype_flag,Sa,dMa,ba,m2a);
      W=W+log(Wl)/(Nr+1);
  end
  W=exp(W);
  W=W./sum(W,1);
...
function [W]=getEW(M,m1,b,kd,S, GRtype_flag,NLEtype_flag,Sa,dMa,ba,m2a)
  
  % Get the vector of magnitude jumps.
  dM=abs(diff([m1,M]));
  ...
  for k=1:Nm
      ...
      % Add the negative log-likelihood for M or dM.
      if(strcmpi(GRtype_flag,'M'))
          LL(k,:)=log(GR_MFD(M,m1,m2,0,b,'norm'));
      elseif(strcmpi(GRtype_flag,'dM'))
          LL(k,:)=log(GR_dM0(dM,m2-m1,0,b,'norm'));
      elseif(strcmpi(GRtype_flag,'both'))
          LL(k,:)=log(GR_MFD(M,m1,m2,0,b,'norm'))/2;
          LL(k,:)=LL(k,:)+log(GR_dM0(dM,m2-m1,0,b,'norm'))/2;
      end
      ...
      for i=1:kd
          ...
          I=D(i).I;
          if(strcmpi(NLEtype_flag,'comp'))
              LL(k,I)=GR_NLEtable(D(i).dMlrg,0,m2(I)-D(i).Mlrg,0,b(I),i,Sa,dMa,ba,m2a); % Composite form.
          elseif(strcmpi(NLEtype_flag,'approx'))
              LL(k,I)=LL(k,I)-log(GR_MFD(D(i).dMlrg,0,m2(I)-D(i).Mlrg,0,i*b(I),'norm')); % Approximate.
          elseif(strcmpi(NLEtype_flag,'both'))
              LL(k,I)=(LL(k,I)-log(GR_MFD(D(i).dMlrg,0,m2(I)-D(i).Mlrg,0,i*b(I),'norm')))/2; % Approximate.
              LL(k,I)=LL(k,I)+GR_NLEtable(D(i).dMlrg,0,m2(I)-D(i).Mlrg,0,b(I),i,Sa,dMa,ba,m2a)/2; % Average with the composite form.
          end
      end
  end
  LL=cumsum(LL,2);
...
% Makes index permutation, with the given Mlrg constraints.
